...
class TenantAnalyticsFilter(FilterSet):
    ...
    billing_plan = CharFilter(
        field_name='payments_tenantbillingsubscriptions__billing_plan__slug',
        lookup_expr='iexact'
    )
    billing_status = CharFilter(
        field_name='payments_tenantbillingsubscriptions__status',
        lookup_expr='iexact'
    )
...
class TenantAnalyticsSerializer(serializers.Serializer):
    ...
    def _active_billing_sub(self, obj):
        ...
        Avoids extra DB hits because the queryset prefetches payments_tenantbillingsubscriptions.
        """
        subs = getattr(obj, '_prefetched_billing_subs', None)
        if subs is None:
            subs = list(obj.payments_tenantbillingsubscriptions.all())
...
def _build_analytics_queryset():
    ...
    qs = Tenant.objects.annotate(
        ...
        'payments_tenantbillingsubscriptions__billing_plan',
    ).order_by('-created_at')
